...
# ------------------ Textual representations ------------------
    ...
    def as_mixed_number(self) :
        ...
        reste = abs(self.__num) % abs(self.__den)

        if reste == 0:  # La fraction est entière
            return str(partieEntier)
        elif abs(self.__num) < abs(self.__den):  # Fraction propre (moins de 1)
            return f'{reste}/{abs(self.__den)}'
        else:  # Fraction mixte
            return f'{partieEntier} + {reste}/{abs(self.__den)}'
